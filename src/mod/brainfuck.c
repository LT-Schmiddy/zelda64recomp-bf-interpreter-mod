...
BrainfuckState * brainfuck_state() {
	BrainfuckState *state = (BrainfuckState *) recomp_alloc(sizeof(BrainfuckState));
	state->root = 0;
	state->head = 0;
	return state;
}
 ...
 */
BrainfuckExecutionContext * brainfuck_context(unsigned char* tape, int size) {
	if (size < 0) {
...
BrainfuckInstruction * brainfuck_parse_substring_incremental(char *str, int *ptr, int end) {
	...
	BrainfuckInstruction *root = (BrainfuckInstruction *) recomp_alloc(sizeof(BrainfuckInstruction));
	...
	for (; *ptr < end && (c = str[*ptr]); (*ptr)++) {
			...
			instruction->next = (BrainfuckInstruction *) recomp_alloc(sizeof(BrainfuckInstruction));
...
BrainfuckInstruction * brainfuck_parse_character(char c) {
	BrainfuckInstruction *instruction = (BrainfuckInstruction *) recomp_alloc(sizeof(BrainfuckInstruction));
	instruction->next = 0;
	instruction->loop = 0;
	instruction->difference = 1;
	switch(c) {
	...
	while (instruction != NULL && instruction->type != BRAINFUCK_TOKEN_LOOP_END) {
		...
		case BRAINFUCK_TOKEN_BREAK: {
			...
			for (index = low; index < high; index++) {
				recomp_printf("%i\t", index);
			}
			recomp_printf("\n");
			for (index = low; index < high; index++) {
				recomp_printf("%d\t", context->tape[index]);
			}
			recomp_printf("\n");
			for (index = low; index < high; index++) {
